...
sudo docker run --rm --name artifact-diff-${coll} -v ${PWD}:/usr/src/app --user $(id -u):$(id -g) -e HOME=/usr/src/app ${IMAGE} python ${SCRIPT} ${coll} ${caom_resource} ${si_resource} || exit $?
